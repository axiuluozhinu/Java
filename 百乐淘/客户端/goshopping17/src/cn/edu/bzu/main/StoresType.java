...
public class StoresType extends ListActivity implements OnItemClickListener {
    /** Called when the activity is first created. */
 ListView listViewAll;
 StoresData storesdata=new StoresData();
 int length=0;
 Stores[] stores=storesdata.getStoresType(); 
 String[] type=null;
 String[] BOYTYPE = null;
    @Override
  public void onCreate(Bundle savedInstanceState) {
    	...
    	for(int i=0;i<type.length;i++){
    		type[i]=stores[i].getType();
    	}
    	for(int i=0;i<BOYTYPE.length;i++){
    		
			BOYTYPE[i]=stores[i].getType();
		}
    	...
    	setListAdapter(new SimpleAdapter(StoresType.this, getDate(), 
    ...
    private OnClickListener menu=new OnClickListener() {
		...
		switch (menu.getId()) {
		case R.id.menu1:
			Intent intent1=new Intent(StoresType.this,Index.class);
			intent1.putExtra("tltle", "首页");
			startActivity(intent1);				
			break;		
		case R.id.menu2:
			Intent intent2=new Intent(StoresType.this,Type.class);
			intent2.putExtra("tltle", "分类");
			startActivity(intent2);					
			break;
		case R.id.menu3:
			Intent intent3=new Intent(StoresType.this,StoresType.class);
			intent3.putExtra("tltle", "商家列表");
			startActivity(intent3);					
			break;
		case R.id.menu4:
			Intent intent4=new Intent(StoresType.this,Maps.class);
			intent4.putExtra("tltle", "百乐淘");
			startActivity(intent4);					
			break;
		case R.id.menu5:
			Intent intent5=new Intent(StoresType.this,More.class);
	...
	public void onItemClick(AdapterView<?> arg0, View arg1, int arg2, long arg3) {
		for (int i = 0; i < BOYTYPE.length; i++) {
			if(i==arg2){
				Intent intent1=new Intent(StoresType.this,StoresName.class);
				intent1.putExtra("index", type[i]);
				startActivity(intent1);		
		}}
